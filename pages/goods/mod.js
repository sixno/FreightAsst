...
Page({
  ...
  onLoad: function (options) {
    ...
    app.api_request('goods/item', options, function (res) {
      if (res.out == 1) {
        ...
        data.formData.goods_id = res.data.id;
        data.formData.goods_name = res.data.name;
        data.formData.goods_cover = res.data.cover;
        data.formData.goods_content = res.data.content;

        for (var i in res.data.size) {
          data.formData['goods_size_model_' + i] = res.data.size[i].model;
          data.formData['goods_size_unit_' + i] = res.data.size[i].unit;
        }

        data.files = [{url: res.data.cover}];

        data.name = res.data.name;
        data.content = res.data.content;
        data.sizes = res.data.size;
  ...
  uplaodFile(files) {
    return new Promise((resolve, reject) => {
      app.api_upload('cover', files.tempFilePaths[0], function (res) {
        console.log(res);
        if (res.out == 1) {
  ...
  submitForm: function () {
    ...
    this.selectComponent('#form').validate((valid, errors) => {
      ...
      if (!valid) {
        ...
        app.api_request('goods/mod', data, function (res) {
          if (res.out == 1) {
            if (app.dat_page) {
              var index = null;
              var update_data = {};
